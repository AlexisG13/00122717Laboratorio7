...
enum parameters {TAU,K_CTE,LAMBDA,DELTA,ETA,EXTERNAL_FORCE_X,EXTERNAL_FORCE_Y};
...
class mesh{
    ...
    public:
        void setParameters(float tau,float k, float lambda, float delta, float eta, float f_x, float f_y){
            parameters[TAU]=tau;
            parameters[K_CTE]=k;
