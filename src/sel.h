...
Matrix createLocalK(int e,mesh &m){
    ...
    float tau,k,lambda,delta,Ae,J,D;
    
    //Componentes de K
    // [ A+K  G ]
    // [  D   0 ]
    Matrix matrixA,matrixK,matrixG,matrixD,matrixC;
    ...
    productRealMatrix(2*J*lambda/(18*D),productMatrixMatrix(g_matrix,productMatrixMatrix(Alpha,BPrima,2,2,3),6,2,3),matrixG);

    //matrixC
    delta = m.getParameter(DELTA);
    productRealMatrix(-(delta*Ae)/(8*(D*D)),productMatrixMatrix(Betat,productMatrixMatrix(Alphat,productMatrixMatrix(Alphat, BPrima, 2,2 ,3) ,2,2,3),6,2,3),matrixC);
    ...
    ubicarSubMatriz(K,0,5,6,8,sumMatrix(matrixG,matrixC,6,3));
...
Vector createLocalb(int e,mesh &m){
    ...
    b.push_back((eta*J)/6); b.push_back((eta*J)/6); b.push_back((eta*J)/6);
...
void assemblyb(element e,Vector localb,Vector &b,int nnodes){
    ...
    int index2 = e.getNode2() - 1;
    int index3 = e.getNode3() - 1;
    int index4 = index1+nnodes, index5 = index2+nnodes, index6 = index3+nnodes, index7 = index4+nnodes,
                 index8 = index5+nnodes, index9 = index6+nnodes;
